    ...
    const workflowList = this.props.workflows.map((workflow, index) => {
      ...
      return(
          <div key={id} className="Workflow">
            <button className="button" onClick={() => this.props.dispatch(deleteWorkflowForm(id))}>Delete Workflow</button>
            <h1 className="WorkflowTitle">{title}</h1>
            <TaskList
              taskArr={tasks}
              workflowId={id}
              />
            <button className="button" onClick={() => this.props.dispatch(addTask(id))}>New Task</button>
